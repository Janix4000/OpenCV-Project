...
template <typename Preprocessor>
bool DocumentCropper<Preprocessor>::get_cropped_document(cv::Mat& dst, cv::Size desired_size /*= { default_height, default_width }*/) const
{
...
template <typename Preprocessor>
bool DocumentCropper<Preprocessor>::is_document_found() const
{
	return failed_tries < max_no_found_tries && !corners.empty();
}

template <typename Preprocessor>
void DocumentCropper<Preprocessor>::show_preprocess() const
{
	corner_finder.show_preprocess();
}

template <typename Preprocessor>
cv::Mat DocumentCropper<Preprocessor>::get_warp(int w, int h) const
{
...
template <typename Preprocessor>
bool DocumentCropper<Preprocessor>::find_document(const cv::Mat& src)
{
